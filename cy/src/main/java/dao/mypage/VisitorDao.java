...
public class VisitorDao extends DB {
	...
	public boolean visitorwrite(Visitor visitor) {
		...
		try {
			...
			System.out.println("방명록 작성 실패 :" + e);
		}
		return false;
	} // 방명록 등록 메소드 end
		// -----------------------------------------------------------------------------------------
	...
	public ArrayList<Visitor> visitorlist(int startrow, int listsize, int user_no) {
		...
		try {
			...
			rs = ps.executeQuery();
			while (rs.next()) {
				Visitor visitor = new Visitor(rs.getInt(1), rs.getInt(2), rs.getString(3), rs.getString(4),
						rs.getString(5));
				visitors.add(visitor);
			}
			return visitors;
		} catch (Exception e) {
			System.out.println("페이징 연습 실패 : " + e);
		}
		return null;
	...
	public ArrayList<Visitor> visitorlist(int user_no) {
		...
		try {
			...
			System.out.println("전체 게시물조회 실패 : " + e);
		}
		return null;
	}
	...
	public boolean updatevisitor(Visitor visitor) {
		...
		try {
			...
			System.out.println("방명록 수정 실패 : " + e);
		}
		return false;
	}
	...
	public boolean deletevisitor(int visitor_no) {
		...
		try {
			...
			System.out.println("방명록 삭제 실패 : " + e);
		}
		return false;
	}
	...
	public int visitorcount() {
		...
		try {
			...
			System.out.println("페이징 처리 실패 :" + e);
		}
		return 0;
	}
	...
	public int getTotalVisitor(int user_no) {

		String sql = "select count(*) from visitor where user_no=" + user_no;
