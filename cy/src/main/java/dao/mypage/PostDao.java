...
public class PostDao extends DB {
	...
	public boolean postupdate(Post post) {
		String sql = "update bpost set title=? , content=? " + "where bpost_no = ?";
		try {
			ps = con.prepareStatement(sql);
			ps.setString(1, post.getTitle());
			ps.setString(2, post.getContent());
	...
	// 전체 사진첩에 작성된 게시물 총 개수 가져오기

	public int getTotalPost(int user_no) {

		String sql = "select count(*) from bpost where user_no="+user_no;
