...
<body>
			...
			<div class="col-md-6 offset-3">
				<form action="../../controller/user/signUpController.jsp" method="post" id="signUpForm" enctype="multipart/form-data" onsubmit="return signUpFormCheck()">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="signId" name="signId" onchange="signUpFormCheck();" />
						<small id="idHelp" class="" style="color: orange">아이디는 10글자를 넘을 수 없습니다.</small>
					</div>
					<div class="form-group">
						<label for="signPassword">비밀번호</label>
						<input type="password" class="form-control" id="signPassword" name="signPassword" onchange="signUpFormCheck();" style="font-family: '나눔';" />
					...
					<div class="form-group">
						...
						<input type="password" class="form-control" id="signPasswordConfirm" name="signPasswordConfirm" onchange="signUpFormCheck();" style="font-family: '나눔';" />
						<small id="passwordHelp" class="" style="color: orange"></small>
					</div>
					<div class="form-group">
						<label for="signName">이름</label>
						<input type="text" class="form-control" id="signName" name="signName" onchange="signUpFormCheck();" />
						<small id="nameHelp" class="" style="color: orange">이름엔 특수문자가 들어갈 수 없고 최대 10글자를 넘을 수 없습니다.</small>
					</div>
					<div class="form-group">
						<label for="signPhone">핸드폰번호</label>
						<input type="text" class="form-control" id="signPhone" name="signPhone" onchange="signUpFormCheck()" placeholder="핸드폰 번호를 입력하세요. " />
					...
					<div class="form-group">
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" name="genderCheck1" id="genderCheck1" value="M" onchange="signUpFormCheck();" />
							<label for="genderCheck1">남성</label>
						</div>
						<div class="form-check form-check-inline">
							<input class="form-check-input" type="radio" name="genderCheck2" id="genderCheck2" value="F" onchange="signUpFormCheck();"  />
					...
					<div class="row">
						<div class="col-md-7">
							<div class="form-group">
								...
								<input type="file" class="custom-file" id="signPic" name="signPic" onchange="signUpFormCheck();" />
					...
					<div class="form-group">
						<label for="">나이</label>
						<select name="signAge" id="signAge" class="custom-select" onchange="signUpFormCheck();">
							<%
							for (int i = 10; i < 60; i++) {
							%>
							<option value="<%=i%>">
					...
					<div class="form-group">
						...
						<input type="text" class="form-control" id="signNickname" name="signNickname" onchange="signUpFormCheck();" />
   ...
   $(function signUpFormCheck() {
		...
		// 패스워드 [ 두 패스워드 동일한지 검사 ]
		if (!pwj.test(signPassword) || !pwj.test(signPasswordconfirm)) {
